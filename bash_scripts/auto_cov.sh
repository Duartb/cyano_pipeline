...
echo ""; printf "\nRunning BBMap on $res assembled contigs files to check coverage ($1 threads):\n\n"
...
do
  ...
  # Writing run log
 echo "$(date) [BBMAP_COVERAGE] bbmap.sh in1=$r1 in2=$r2 ref=./outputs/spadesOut/$base_name/contigs.fasta covstats=./outputs/coverages/$cov path=./outputs/coverages t=$1 : done" >> ./outputs/commands.log

 # Running Quast
 bbmap.sh in1=$r1 in2=$r2 ref=./outputs/spadesOut/$base_name/contigs.fasta covstats=./outputs/coverages/$cov path=./outputs/coverages t=$1 >>./outputs/console.log 2>> ./outputs/console.log;
done
