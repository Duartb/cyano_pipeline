#!/usr/bin/env bash
set -e
mkdir -p ./outputs/krakenOut/final
...
do
  ...
  # Writing run log
  echo -e "$(date) [KRAKEN2] kraken2 --db ~/tools/kraken2/ --paired $f1 $f2 --report ./outputs/krakenOut/final/$outR --output ~/newData/outputs/krakenOut/final/$outK --threads $1 : done" >> ./outputs/commands.log

  # Writing run log
  kraken2 --db ~/tools/kraken2/ --paired $f1 $f2 --report ./outputs/krakenOut/final/$outR --output ~/newData/outputs/krakenOut/final/$outK --threads $1 >> ./outputs/console.log 2>> ./outputs/console.log;
done
